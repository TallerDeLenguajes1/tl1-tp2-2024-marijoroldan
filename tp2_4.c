...
struct compu
{
    int velocidad, anio, cantidad; // Velocidad entre 1 y 3 Ghz, Año entre 2015 y 2023, cantidad de nucleos entre 1 y 8.
...
void cargarDatos(struct compu *Pdatos)
{
    ...
    for (int i = 0; i < MAXPC; i++)
    {
        ...
        printf("\nIndique el Año entre 2015 y 2023: \n");
        do
        {
            scanf("%i", &(Pdatos->anio));
            fflush(stdin);
            if (Pdatos->anio < 2015 || Pdatos->anio > 2023)
            {
                printf("\nIngrese un año valido!\n", 164);
            }
        } while (Pdatos->anio < 2015 || Pdatos->anio > 2023);
